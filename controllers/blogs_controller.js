...
return !user || !blog || user.id !== blog.user.toString() ? null : user;
};
...
return response.status(400).json({ message: "missing blog parameters" });

if (!blog["likes"] || blog["likes"] == "") blog["likes"] = Number(0);
...
const user = request.body.user ? await blogOwner(request.body.user, id) : null;
if (!user) return response.status(401).json({ error: "not Authorized" });
try {
...
} else return response.status(404).json({ message: "No blog found" });
} catch (error) {
return response.status(500).json({ message: error.message });
}
});
