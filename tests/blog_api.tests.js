...
const blogHelper = require("../utils/blogs_test_helper");
beforeAll(async () => {
...
const res = await api.get("/api/blogs");
expect(res["_body"].length).toBe(blogHelper.initBlogs.length + 1);
});
let id2Delete = "";
...
await api.put(`/api/blogs/${id2Delete}`).send(newBlog).expect(202);
});
